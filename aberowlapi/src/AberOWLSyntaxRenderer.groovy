...
public class AberOWLSyntaxRenderer extends AbstractRenderer implements OWLObjectVisitor {
    ...
    protected void write(@Nonnull Set<? extends OWLClassExpression> objects,
        ...
        for (Iterator<? extends OWLObject> it = sort(objects).iterator(); it
            ...
            if (!first) {
                ...
                write("<span id='man-and'>"+ AND + "</span>");
            }
            first = false;
    ...
    private void writeRestriction(
        ...
        write("<span id='man-keyword'>"+keyword+"</span>");
    ...
    private <V extends OWLObject> void writeRestriction(
        ...
        p.accept(this);
        write("<span id='man-hasvalue'>"+VALUE+"</span>");
        restriction.getFiller().accept(this);
    }
    ...
    private <F extends OWLPropertyRange> void writeRestriction(
        ...
        p.accept(this);
        write("<span id='man-keyword'>"+keyword+"</span>");
        write("<span id='man-number'>"+Integer.toString(restriction.getCardinality())+"</span>");
        writeSpace();
        if (restriction.getFiller() instanceof OWLAnonymousClassExpression) {
            write("<span>(</span>");
        }
	...
	write(
	    "<span id='man-owlclass' data-iri='"
	    + ce.getIRI().toString() + "'><a href='#/Browse/<"
	    + URLEncoder.encode(ce.getIRI().toString(), 'UTF-8')
	    + ">'>" + getShortFormProvider().getShortForm(ce) + "</a></span>");
    }
    ...
    public void visit(@Nonnull OWLObjectComplementOf ce) {
      write("<span id='man-not'>" + NOT + "</span>" + ce.isAnonymous() ? " " : "");
        if (ce.isAnonymous()) {
            write("<span>(</span>");
        }
    ...
    public void visit(@Nonnull OWLObjectHasSelf ce) {
        ...
        write("<span id='man-keyword'>"+SOME+"</span>");
        write("<span id='man-self'>"+SELF+"</span>");
    }

    @Override
    public void visit(@Nonnull OWLObjectOneOf ce) {
        write("<span id='man-set'>{</span>");
    ...
    public void visit(OWLObjectProperty property) {
      write("<span title='"+property.getIRI()+"' id='man-property' data-iri='"+property.getIRI()+"'>"+getShortFormProvider().getShortForm(property)+ "</span>");
    }
    ...
    public void visit(OWLNamedIndividual individual) {
      write("<span title='"+individual.getIRI()+"' id='man-individual' data-iri='"+individual.getIRI()+"'>"+getShortFormProvider().getShortForm(individual)+ "</span>");
    }
    ...
    public void visit(@Nonnull IRI iri) {
      write("<span title='"+iri+"' id='man-iri' 'data-iri="+iri.toString()+"'>"+iri.toQuotedString()+ "</span>");
    }
